...
4. 两个资源都合格且总分不同时，新资源必须至少领先 `min_score_diff` 才会洗版。
...
每条质量评分规则都可以在规则顶层配置 `min_score_diff`，限制普通总分领先幅度不足时触发洗版。例如：
...
以上配置表示新文件总分必须至少领先现有文件 `10` 分才会洗版；领先 `9.99` 分会保留现有文件，恰好领先 `10` 分则允许洗版。

- 省略 `min_score_diff` 时默认使用 `0.1`，与旧版本行为一致，旧 YAML 无需修改。
- 配置为 `0` 时，任意正分差都允许洗版；总分完全相同不会因为数值门槛直接触发替换。
- 该字段只接受大于或等于 `0` 的有限数字，负数、NaN 和无穷值无法保存。
- 合格资源替换不合格资源不受最低分差限制；总分完全相同时，仍按音轨/字幕 `priority` 和 `preferred_tracks` 破同分。
- 阈值属于具体评分规则。按分类目录选择不同评分规则时，会自动使用实际命中规则自己的阈值。
- YAML 字段名是 `min_score_diff`，接口 JSON 中对应 `minScoreDiff`。
...
| `subtitle` | `[PGS 简体中文, ASS 简体中文, SRT 简体中文, PGS, ASS, SRT, VTT]` | 字符串按字幕摘要评分；可靠关联的外挂字幕也使用这套旧式优先级，需要限定语言、来源或标记时使用下方轨道条件 |
...
| `formats` | string[] | 字幕格式 | `[PGS, ASS, SRT, VTT]` |
...
系统以**当前目录**为单位，一次建立视频和外挂字幕的共享关联结果。字幕是视频的伴随文件，不是独立整理对象：不扫描子目录、不跨目录关联；当前目录没有视频时，字幕不会被整理或评分。

#### 格式门控

最终可评分的外挂字幕格式为“当前任务有效的 `metadataExts`”与系统可评分字幕格式的交集：

| 格式 | 行为 |
|------|------|
| `.srt` / `.ass` / `.ssa` / `.sub` / `.sup` / `.vtt` | 配置启用、且成功关联后参与评分和转移 |
| `.idx` | 仅随已关联的同名 `.sub` 转移，不单独评分 |
| `.smi` / `.ttml` | 可按配置转移，暂不评分 |
| `.nfo` / `.jpg` / `.json` 等 | 普通元数据，不进入字幕评分 |

`.vtt` 不在默认元数据后缀中，不会自动启用；用户把它加入全局或方案的 `metadataExts` 后才生效。方案未自定义后缀时使用全局配置，方案自定义后以方案配置为准。删除某个后缀会同步停止该格式的关联、评分和转移。

#### 三层匹配顺序

系统按以下层级依次匹配，只要某一层得到唯一候选就停止；某层得到多个候选时直接判定歧义，不会降级到下一层碰运气。一个字幕最多归属一个视频。

1. **完整 stem**：媒体基名相同，或字幕只在完整视频 stem 后增加语言、`forced` 等字幕描述。
2. **规范化 stem**：统一全半角、大小写、标点和连续分隔符后相同。
3. **文件名媒体身份**：分别提取双方的标题、年份、季、集，再比较结构化身份。

```text
Movie.2024.2160p.WEB-DL.mkv
Movie.2024.2160p.WEB-DL.zh-CN.srt  -> 完整 stem

Show.Name-S01E03.mkv
show_name_S01E03.chs.ass           -> 规范化 stem

Show.S01E03.1080p.WEB-DL.x265.mkv
Show.S01E03.zh-CN.ass               -> 文件名媒体身份
```

标题比较会统一全半角、大小写、标点和连续分隔符，但不会使用编辑距离做模糊相似度匹配。因此 `Movie2.srt`、`MovieExtra.srt` 这类前缀相似但身份不同的文件不会被强行认领。

#### 剧集匹配限制

- 双方都解析出季、集时，季号和集号必须一致；冲突时直接拒绝，不使用标题相似度挽救。
- `S01E03.chs.ass` 没有剧名时，可以匹配当前目录内唯一的 `S01E03` 视频。
- `E03.chs.ass` 只有集号时，仅当当前目录所有可识别视频都属于同一季，才允许推断季号。
- 字幕解析出剧名时，必须与视频的任一标题一致。
- 同一季集有多个视频版本，而字幕没有版本信息时，候选不唯一，判定为歧义。
...
上例字幕不能确定属于哪个版本，因此不关联。

#### 电影匹配限制

- 字幕标题必须与视频的任一标题一致。
- 双方都有年份时，年份必须一致。
- 字幕没有年份时，只有当前目录内存在唯一同标题电影才接受。
- 只有年份、没有有效标题时不关联。
- 不使用“目录只有一个视频就强制归属”的兜底。

```text
Dune.Part.Two.2024.2160p.WEB-DL.mkv
Dune.Part.Two.2024.zh-CN.ass        -> 匹配
Dune.Part.Two.zh-CN.ass             -> 同标题候选唯一时匹配
Dune.1984.zh-CN.ass                 -> 年份冲突，拒绝
```

#### 关联结果如何复用

成功关联后，同一份目录级结果会同时用于来源去重、首次整理资格判断、目标洗版评分、伴随转移、重命名和旧文件处理，各阶段不会重新猜测归属。

宽松关联的字幕按“目标视频 stem + 原字幕语言/属性后缀 + 字幕扩展名”重命名：
...
无法唯一归属、身份冲突或当前目录没有视频的字幕：

- 不评分；
- 不转移、不重命名；
- 不随来源去重或洗版删除；
- 标记为本次任务的受保护文件，阻止空目录清理误删；
- 在日志中记录字幕身份、候选视频以及歧义或拒绝原因。

如果主视频没有可靠的文件名媒体身份，只使用完整 stem 的原有严格规则，不启用规范化或媒体身份宽松匹配。

#### 评分兼容性

可靠关联并且格式可评分的外挂字幕，会同时进入以下两套规则：

- 结构化轨道条件：支持 `languages`、`formats` 和 `source: external`。
- 旧式字符串优先级：例如 `PGS > ASS > SRT` 或 `PGS 简体中文 > ASS 简体中文 > SRT 简体中文`。

格式别名按同一字幕族处理：`.sup` 可匹配 `SUP` 或 `PGS`，`.vtt` 可匹配 `VTT` 或 `WebVTT`。未写语言标记的外挂字幕语言为 `und`。只有当前 `metadataExts` 允许且已成功关联的字幕才参与评分。
...
在「自定义规则 → TMDB 规则」中维护 TMDB 元数据覆盖和自动识别黑名单。两个列表可以写在同一份 YAML 中；页面会分别显示当前生效的覆盖规则和黑名单数量。
...
黑名单**只影响自动识别**。手动 TMDB 搜索、手动选择和手动整理仍可使用被列出的 ID。保存或订阅重载后，短期 TMDB 匹配缓存也会重新复核黑名单，旧的错误命中不会继续被自动采用，无需手动清除 TMDB 元数据缓存。
...
```
mr 渲染词替换 → 文件名解析 → TMDB 匹配 → 季集映射(MP) → 命名模板 → output_mr 输出渲染
```

### 规则管理

- **本地规则**：在「自定义规则」页面直接编辑 YAML
- **TMDB 规则**：在「自定义规则 → TMDB 规则」维护覆盖和黑名单；页面分别显示两类规则的生效数量
...
所有规则（分类、命名、质量、自定义规则、TMDB 规则）都支持从远程 URL 订阅。MediaTidy 使用**统一订阅管理**系统：
...
| `tmdb_override` | TMDB 规则订阅（`tmdb_override` 覆盖和 `tmdb_blacklist` 自动识别黑名单） |
