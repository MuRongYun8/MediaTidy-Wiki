...
- 文件名和媒体信息都无法提供该项的值时，不会自动把缺失方算作 0 分。具体处理方式见下方“缺少信息时到底怎么评分”。
...
> **没取到信息，不等于确认没有。只有确认没有时才按 0 分；无法判断时，这一项双方都不比较。**
...
| 没有运行 FF、FF 失败，或旧缓存没有 `subtitleTracks` | 无法判断有没有字幕，不把它当成无字幕；这次比较中双方都不计算字幕项 |
...
| 文件名没有分辨率，而且没有成功取得完整 FF | 无法判断分辨率，双方都不计算分辨率项 |
...
| 视频编码、色深或帧率没有采集到 | 无法判断，双方都不计算对应项目 |
| 完整 FF 已采集，但确认没有对应视频信息 | 确定没有，对应项目按 0 分参与 |
| 完整音轨或字幕清单为空 | 确定没有音轨或字幕，对应项目按 0 分参与 |
| 旧缓存没有音轨或字幕清单 | 无法判断，不按 0 分，也不会让信息齐全的一方单方面得分 |
| 文件大小、容器总码率或视频流码率取不到有效数值 | 无法判断，双方都不计算该数值评分项 |
| 文件确实没有扩展名 | 确定没有扩展名，容器格式按 0 分参与 |

#### 普通偏好和硬性条件的区别

- `priority` 和 `preferred_tracks` 是普通偏好。任一方的完整轨道清单无法判断时，双方都不计算这个音轨或字幕评分项。
