...
- 文件名和媒体信息都无法提供该项的值时，不会自动把缺失方算作 0 分；具体按下方三态规则处理。
...
洗版是按两个文件成对比较。每个评分输入先归入三种状态：

- **有值**：例如 `2160p`、`HDR10`、两条字幕轨，正常评分。
- **确认不存在**：例如完整 FF 结果里的 `subtitleTracks: []`，表示确实没有字幕，是可靠事实，该维度正常参与且通常为 0 分。
- **未知**：例如没有执行 FF、探测失败，或旧缓存尚未采集轨道清单。未知不等于没有，普通维度在任一方未知时对双方同时忽略。

| 维度 | 有值或默认语义 | 确认不存在 | 任一方未知时 |
|------|----------------|------------|--------------|
| `source` | 文件名识别；识别不到使用 `Unknown` | 不使用 | 始终参与 |
| `hdr` | FFprobe/文件名明确值优先；无 HDR 标签按 `SDR` | 完整 FF 确认无有效视频 | 始终有默认语义；无有效视频计 0 |
| `resolution` | FFprobe 或文件名 | 完整 FF 无有效视频流 | 双方忽略 |
| `video_codec` / `color_depth` / `frame_rate` | FFprobe 或文件名 | 完整 FF 确认字段不存在 | 双方忽略 |
| 旧式 `audio_codec` / `subtitle` 摘要 | FFprobe 摘要或可用的文件名回退 | 完整轨道清单为空 | 双方忽略 |
| 结构化音轨/字幕偏好、`preferred_tracks` | 完整轨道清单 | 完整清单为空 | 双方忽略整个维度 |
| `required` / `min_tracks` / `max_tracks` | 完整轨道清单 | 用空清单正常验证 | 无法验证，资源判不合格 |
| `file_extension` | 从实际文件名/路径提取 | 无扩展名 | 正常始终参与；无扩展名计 0 |
| `file_size` / `bitrate` / `video_bitrate` | 有效正数 | 不使用 | 双方忽略 |

::: warning
“没有字幕”和“字幕信息未知”不同。有字幕可以正常胜过确认无字幕；但不能仅因为一方旧缓存没有 `subtitleTracks` 就把它当成无字幕。同样规则也适用于音轨。
:::
