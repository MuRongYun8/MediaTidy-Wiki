...
::: tip 排除自动识别候选
如果某个 TMDB 条目会在自动识别中造成误匹配，请在[规则引擎的 TMDB 规则](/features/rules#tmdb-规则覆盖与自动识别黑名单)中使用 `tmdb_blacklist`。它不会删除这里的缓存，也不会影响手动 TMDB 搜索、选择或整理；保存规则后，短期匹配缓存中的旧命中会自动复核并不再被采用。
:::
