...
地址填写 Emby 基础地址即可，不需要手动补 `/emby`。连接测试和「同步信息」还会校验媒体库访问并保存稳定的 Server ID；如果页面提示历史重复 Server ID，请先处理重复项，避免 Webhook 事件路由到错误的服务器。
...
点击「检测路径映射」按钮可自动对比目录结构并推荐映射配置。检测只使用已保存的 STRM 输出目录，不会把整理目标目录当作可播放路径；多个 STRM 输出目录会分别显示验证状态。请先确认结果为「已验证」或「可应用映射」再保存，无法唯一判断的目录需手动核对。
