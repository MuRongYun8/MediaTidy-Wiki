...
使用 Token 认证时，建议在 CD2 中将令牌可访问目录设为 CD2 根目录 `<code>/</code>`，并按实际功能授予权限：仅查看或生成 STRM 至少需要列出、读取和下载；整理、移动、复制或删除还需要对应的写入与删除权限。连接测试成功只代表令牌有效；如果只授权某个子目录，来源发现和部分路径操作仍可能不完整。
...
「CD2 读取强度」控制 STRM、FUSE 和媒体探测的读取并发，默认是「均衡 75%」。它不是一组手填的全局 QPS，而是按 CD2 实际返回的来源配置分别调度：来源键为 `cloudName + userName`，同一账号的多个根路径共享额度，不同账号或不同本地来源分别调度。

可选档位为「低档 50%」「均衡 75%」和「满速 100%」，满速也不会超过 CD2 当前设置。页面会列出来源根路径、云端 API QPS 和最大下载线程数；CD2 未提供 API QPS 时不会凭空加上限制，但文件读取仍会按下载线程数排队。点击「刷新来源」可重新读取 CD2 配置。
...
保存路径映射时，不能让 115 映射与通用挂载占用同一段 CD2 路径树，否则虚拟路径无法唯一逆向解析。历史上已经存在的重叠配置仍可按兼容规则读取，但后续保存需要先调整根路径。
...
API 调用延迟用于控制对 115 服务器的请求频率，避免触发限频或风控；CD2 的文件读取并发和云端 API QPS 请使用上面的「CD2 读取强度」及 CD2 自身来源配置管理。
...
115「生活事件」实时模式始终保留至少 5 秒的请求间隔，即使全局倍率或单项延时更低，也不会连续请求。
